...
dict(id=52, date=(1401, 12, 19), topic='اسنپ', cats=['تپه‌نوردی', 'وب‌نوردی'],
     ...
     body=[
      ...
      (P, 'یه نکته‌ای که خودم انتظارش رو نداشتم این بود که پخش نقاط اصلاً یکنواخت نیست. '
          'یعنی محصول در بعضی مناطق تقریباً همه‌جا هست و در بعضی مناطق اصلاً نیست.'),
...
dict(id=53, date=(1403, 2, 8), topic='ابزار وب', cats=['تپه‌نوردی', 'کاربردی'],
     ...
     body=[
      ...
      (P, 'هدف این بود که ببینیم با تصمیم‌های جمعیِ چند صد نفر آدم که هیچ‌کدوم همدیگه رو '
          ...
          'علمی‌ای پشتش نبود.'),
      (P, 'جوابش هم تقریباً همون چیزی شد که حدس می‌زدی: وقتی هر کسی سه تا تغییر در روز داره '
          'و هیچ هماهنگی‌ای بین آدم‌ها نیست، سیستم بیشتر از اینکه به تعادل برسه، نوسان می‌کنه.'),
      (P, 'بعد از یه مدت غیرفعالش کردم.'),
     ]),
...
dict(id=55, date=(1403, 3, 7), topic='ابزار وب', cats=['تپه‌نوردی', 'کاربردی'],
     ...
     body=[
      ...
      (P, 'با FastAPI نوشته شده بود و روی لیارا مستقر شده بود.'),
      (P, 'جالبیش این بود که آدم‌ها خودشون شروع کردن به هماهنگ‌کردن. یعنی بازی هیچ مکانیزم '
          'چتی نداشت، ولی مردم تو کامنت‌ها و توییتر برای همدیگه استراتژی می‌نوشتن.'),
     ]),
...
dict(id=56, date=(1403, 3, 17), topic='امنیت', cats=['تپه‌نوردی'],
     ...
     body=[
      ...
      (P, 'راستش هیجانِ پیدا کردن الگوریتم سایفر، از خودِ سکه‌ها خیلی بیشتر بود. من به این '
          'می‌گم تفریح سالم :)'),
     ]),
...
dict(id=58, date=(1403, 6, 19), topic='اعداد', cats=['تپه‌نوردی'],
     ...
     imgs=[('tg', 160, 0), ('tw', '1833190531683230135', 0)],
     body=[
      ...
      (P, 'اینکه سال‌های آینده هم توی نمودار هست به این خاطره که تعطیلی‌های رسمی از قبل '
          'مشخصن — هم مناسبت‌های شمسی، هم قمری که هر سال حدود ۱۱ روز عقب می‌ره و همین '
          'باعث می‌شه توزیعشون روی ماه‌ها بچرخه.'),
...
dict(id=59, date=(1403, 9, 1), topic='اسنپ', cats=['تپه‌نوردی', 'کاربردی'],
     ...
     body=[
      (P, 'سه سال پیش، دوران سربازی، یه اسکریپت نوشته بودم که بهم می‌گفت اگر چند ده متر '
          'جابه‌جا بشم، کرایه‌ی اسنپ چقدر فرق می‌کنه. اون موقع فقط برای خودم بود و خروجیش '
          'هم یه مشت عدد توی ترمینال.'),
      ...
      (P, 'من کلاً دوران سربازی رو با آنالیز دیتا تونستم از پس هزینه‌ها بر بیام؛ از همین '
          'اسنپ بهینه تا کلوچه‌ای که با خودم می‌بردم.'),
     ]),
...
dict(id=60, date=(1403, 10, 3), topic='اعداد', cats=['تپه‌نوردی'],
     ...
     imgs=[('tg', 253, 0)],
     body=[
      (P, 'داشتم فکر می‌کردم فاصله‌ی زمانی بین هر دفعه که می‌رم سلمونی چقدره. یه چیزی که '
          'هیچ‌وقت بهش فکر نکرده بودم ولی دیتاش رو داشتم.'),
      (P, 'نمودارش رو کشیدم و نتیجه این شد:'),
      (I, 0, 'فاصلهٔ بین هر دو نوبت، و ساعتی که می‌روم'),
...
dict(id=61, date=(1403, 11, 10), topic='زبان فارسی', cats=['تپه‌نوردی', 'وب‌نوردی'],
     ...
     imgs=[('tw', '1884264994889687393', 0)],
     body=[
      (P, 'قبلاً یه بار رفتم سراغ گنجور و کل شعر فارسی رو شخم زدم. این بار سؤالم فرق '
          'می‌کرد: چه کلماتی هست که <strong>همه‌ی</strong> شاعرها، بدون استثنا، تو شعرشون '
          'به کار بردن؟'),
      ...
      (P, 'اینکه لیست انقدر کوتاهه خودش جالبه. یعنی زبان مشترکِ قطعیِ بین حافظ و نیما، '
          'بعد از حذف کلمات دستوری، تقریباً به اندازه‌ی یه جمله‌ست.'),
     ]),
...
dict(id=62, date=(1403, 12, 26), topic='تهران و نقشه', cats=['تپه‌نوردی', 'وب‌نوردی'],
     ...
     body=[
      ...
      (P, 'برای اینکه بهتر نقشه رو درک کنید: ارتفاع ساختمون ۴ طبقه حدود ۱۵ متر در نظر '
          'گرفته شده. نقشه رو هم با kepler.gl رسم کردم — دیتا رو آماده کردم و import کردم.'),
...
dict(id=63, date=(1404, 1, 15), topic='رسانه', cats=['تپه‌نوردی', 'وب‌نوردی'],
     ...
     body=[
      ...
      (P, 'لیستی که در نهایت موند خیلی کوتاه‌تر از چیزی بود که فکر می‌کردم. که خودش '
          'یه چیزی درباره‌ی نحوه‌ی ساخت سریال می‌گه: نگه‌داشتنِ یه روند صعودی در همه‌ی '
          'فصل‌ها، تقریباً اتفاق نمی‌افته.'),
     ]),

dict(id=64, date=(1404, 2, 14), topic='رسانه', cats=['تپه‌نوردی', 'وب‌نوردی'],
     title='ژانرهای IMDb زیر ذره‌بین!',
     tg=255, tw=None,
     imgs=[('tg', 255, 0), ('tg', 257, 0), ('tg', 260, 0)],
     body=[
...
dict(id=65, date=(1404, 2, 19), topic='بازار و مصرف', cats=['تپه‌نوردی', 'کاربردی'],
     ...
     body=[
      ...
      (P, 'هدف؟ هیچی!'),
      (P, 'ولی خب بعد از چند هفته که لاگ جمع شد، خودش تبدیل شد به یه چیز جالب: می‌شه دید '
          'سقفِ قیمت توی دیجی‌کالا چه‌جوری بالا و پایین می‌ره، و ارزون‌ترین محصولِ موجود '
          'معمولاً چه چیزیه.'),
      (P, 'خوبی Cloudflare Worker هم اینه که برای همچین کار کوچیکی اصلاً لازم نیست سرور '
          'داشته باشی. یه cron و یه فایل، تمام.'),
     ]),
...
dict(id=67, date=(1404, 7, 24), topic='امنیت', cats=['کاربردی'],
     ...
     body=[
      ...
      (P, 'یعنی این ربات‌هایی که تا الان برای من کامنت می‌ذاشتن، همه‌شون همین‌جوری بودن. '
          'یه شماره‌ی خارجی می‌ذارن که آدم بهش زنگ بزنه، و همون شماره لوشون می‌ده.'),
      (P, 'قاعده‌ی کلیش اینه: قبل از اینکه بری سراغ فیلترِ هوشمند، ببین دیتاست خودت چه '
          'الگوی احمقانه‌ای داره. معمولاً داره.'),
     ]),
...
dict(id=69, date=(1405, 4, 24), topic='اعداد', cats=['تپه‌نوردی', 'وب‌نوردی'],
     ...
     body=[
      ...
      (P, 'خوبی بلاک‌چین همینه: لازم نیست کسی دیتا بهت بده. تراکنش‌ها عمومی‌ان و فقط باید '
          'بدونی کدوم آدرس مال کیه.'),
      (P, 'جالب‌ترین چیزی که دیدم این بود که وقتی قیمت دلار می‌رفت بالا، خرید مردم زیاد '
          'می‌شد؛ و وقتی قیمت می‌اومد پایین، خریدشون کم می‌شد.'),
      (P, 'یعنی دقیقاً برعکس چیزی که هر توصیه‌ی مالی‌ای می‌گه. ولی خب، آدم‌ها این‌جوری‌ان.'),
     ]),
